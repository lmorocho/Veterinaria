...
// Si no hay usuario logueado, marcar como Invitado
$usuario = 'Invitado';

// Si hay sesión, extraer nombre o marcar como Invitado
if (isset($_SESSION['usuario'])) {
  $usuario = $_SESSION['usuario']['Nombre'] ?? 'Invitado';
}

// Validar si ya está registrado por email + password (desde sesión temporal)
if (!isset($_SESSION['usuario'])) {
  // Si se han guardado temporalmente email/password en sesión para el nuevo usuario
  $email = $_SESSION['email_temp'] ?? '';
  $password = $_SESSION['password_temp'] ?? '';

  if ($email && $password) {
    $tablas = ['Cliente', 'Empleado', 'Proveedor'];
    ...
    foreach ($tablas as $tabla) {
      ...
      if ($res->num_rows === 1) {
        // Ya está registrado, redirigir a login
...
<body>
  ...
  <div class="alert alert-warning text-center fst-italic" role="alert">
    <!--<h4>Reporte de Mascotas de <?php //echo htmlspecialchars($usuario); ?></h4>-->
